// JavaScript для фінансової звітності
// Data and mappings
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const dashboardTabs = document.querySelectorAll('#dashboard-tabs .nav-link');
    if (dashboardTabs && dashboardTabs.length) {
        dashboardTabs.forEach(tab => {
            tab.addEventListener('shown.bs.tab', function (e) {
                const targetTab = e.target.id ? e.target.id.replace('-tab', '') : '';
...
document.addEventListener('DOMContentLoaded', function() {
    // Initialize date labels and slider if missing
    const minDateEl = document.getElementById('min-date-display');
    const maxDateEl = document.getElementById('max-date-display');
    if (minDateEl) minDateEl.textContent = minDateStr;
    if (maxDateEl) maxDateEl.textContent = maxDateStr;

    const ds = document.getElementById('date-slider');
    if (ds && !ds.noUiSlider) {
    ...
    (ds || dateSlider)?.noUiSlider?.on('update', function (values, handle) {
    ...
    function goToPage(page) {
        ...
        // Keep position: no auto-scroll on pagination
    }
    ...
    bubbleG.attr('transform', 'translate(12,12)');
        ...
        console.log('Оновлено типи звітів');
    }
    ...
    const height = (parseInt(revenueSvg.style('height')) || 280) - margin.top - margin.bottom;
        ...
        console.log(`Оновлено графік за ${getPeriodLabel(period)}`);
    }
    ...
    const height = (parseInt(expensesSvg.style('height')) || 280) - margin.top - margin.bottom;
        ...
        console.log(`Змінено вигляд на ${viewLabel}`);
    }
